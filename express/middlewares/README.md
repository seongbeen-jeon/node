...
// static 뒤에 위치하는 미들웨어들은 실행되지 않습니다.
...
next를 사용할 때 아무 인자를 넣지 않는 형태로 사용했지만 특수한 인자를 넣으면 해당 미들웨어로 이동합니다.

현재 확인 가능한 next() 메서드에 인자로 넣을 수 있는 값은 route, err가 있습니다.
**next('route')**를 사용하면 라우터에 연결된 다른 미들웨어를 건너뜁니다.
route가 아닌 다른 값을 인자로 넣으면 에러로 처리 되어 에러처리 미들웨어로 넘어갑니다.
